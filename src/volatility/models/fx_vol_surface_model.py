...
from .vol_surface_model import VolSurfaceModel
from .vol_surface import VolSurfaceSlices, VolStrikeSlice
...
@dataclass
class PolynomialCurve(VolStrikeSlice):
    _coeffs: list[float]
    _delta_atm: float
...
@dataclass
class SplineCurve(LogCubicSplineNatural, VolStrikeSlice):
...
class FXVolSurfaceModel(VolSurfaceModel):
    ...
    def build_QD(self):
        ...
        for (expiry, _), moneyness_vols in implied_vols.items():
            ...
            delta_atm = fx_delta.get_delta_for_moneyness(moneyness=moneyness_atm, dcf=expiry_dcf,
                                                          sigma=vol_atm, delta_type=self._delta_type)
            res = solver.find_fit(self.get_QD_solver, init_guess=[0, 0],
                                    args=(moneyness_vols, delta_atm, vol_atm, expiry_dcf), bounds=bounds)
            curve = PolynomialCurve([vol_atm] + list(res), delta_atm, self._delta_type)
    ...
    def build(self, model_type: VolatilityModelType = None, **kwargs):
        match model_type:
            case VolatilityModelType.SplineM | None:
                return self.build_SM()
            case VolatilityModelType.QuadD:
            ...
            case _:
                raise Exception(f'{model_type} not supported for FX')
